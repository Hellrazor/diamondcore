...
enum __QuestFlags
{
    ...
    // DiamondCore flags for set SpecialFlags in DB if required but used only at server
    QUEST_DIAMOND_FLAGS_REPEATABLE           = 0x01000000,   // Set by 1 in SpecialFlags from DB
    QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT = 0x02000000,   // Set by 2 in SpecialFlags from DB (if required area explore, spell SPELL_EFFECT_QUEST_COMPLETE casting, table `*_script` command SCRIPT_COMMAND_QUEST_EXPLORED use, set from script DLL)
    QUEST_DIAMOND_FLAGS_DB_ALLOWED = 0xFFFFFF | QUEST_DIAMOND_FLAGS_REPEATABLE | QUEST_DIAMOND_FLAGS_EXPLORATION_OR_EVENT,

    // DiamondCore flags for internal use only
    QUEST_DIAMOND_FLAGS_DELIVER              = 0x04000000,   // Internal flag computed only
    QUEST_DIAMOND_FLAGS_SPEAKTO              = 0x08000000,   // Internal flag computed only
    QUEST_DIAMOND_FLAGS_KILL_OR_CAST         = 0x10000000,   // Internal flag computed only
    QUEST_DIAMOND_FLAGS_TIMED                = 0x20000000,   // Internal flag computed only
};
