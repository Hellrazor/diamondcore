...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket & recv_data)
{
    ...
    data << uint32(petitionguid_low);                       // guild guid (in DiamondCore always same as GUID_LOPART(petitionguid)
...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket & recv_data)
{
    ...
    uint32 guildguid;
    uint64 petitionguid;
    recv_data >> guildguid;                                 // in DiamondCore always same as GUID_LOPART(petitionguid)
    recv_data >> petitionguid;                              // petition guid
...
void WorldSession::SendPetitionQueryOpcode(uint64 petitionguid)
{
    ...
    data << uint32(GUID_LOPART(petitionguid));              // guild/team guid (in DiamondCore always same as GUID_LOPART(petition guid)
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket & recv_data)
{
    ...
    data << uint64(petitionguid);                           // petition guid
    data << uint64(_player->GetGUID());                     // owner guid
    data << uint32(GUID_LOPART(petitionguid));              // guild guid (in DiamondCore always same as GUID_LOPART(petition guid)
    data << uint8(signs);                                   // sign's count
