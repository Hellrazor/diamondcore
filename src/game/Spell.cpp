...
void SpellCastTargets::read( ByteBuffer& data, Unit *caster )
{
    ...
    if( m_targetMask & TARGET_FLAG_SOURCE_LOCATION )
    {
        ...
        if(!Diamond::IsValidMapCoord(m_srcX, m_srcY, m_srcZ))
    ...
    if( m_targetMask & TARGET_FLAG_DEST_LOCATION )
    {
        ...
        if(!Diamond::IsValidMapCoord(m_destX, m_destY, m_destZ))
            throw ByteBufferException(false, data.rpos(), 0, data.size());
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_RANDOM_ENEMY_CHAIN_IN_AREA:
        {
            ...
            {
                Diamond::AnyAoETargetUnitInObjectRangeCheck u_check(m_caster, max_range);
                Diamond::UnitListSearcher<Diamond::AnyAoETargetUnitInObjectRangeCheck> searcher(m_caster, tempTargetUnitMap, u_check);
        ...
        case TARGET_RANDOM_FRIEND_CHAIN_IN_AREA:
        {
            ...
            std::list<Unit*> tempTargetUnitMap;
            {
                Diamond::AnyFriendlyUnitInObjectRangeCheck u_check(m_caster, max_range);
                Diamond::UnitListSearcher<Diamond::AnyFriendlyUnitInObjectRangeCheck> searcher(m_caster, tempTargetUnitMap, u_check);
                Cell::VisitAllObjects(m_caster, searcher, max_range);
            }
        ...
        case TARGET_CHAIN_DAMAGE:
        {
            ...
            else
            {
                ...
                std::list<Unit *> tempTargetUnitMap;
                {
                    Diamond::AnyAoEVisibleTargetUnitInObjectRangeCheck u_check(pUnitTarget, originalCaster, max_range);
                    Diamond::UnitListSearcher<Diamond::AnyAoEVisibleTargetUnitInObjectRangeCheck> searcher(m_caster, tempTargetUnitMap, u_check);
                    Cell::VisitAllObjects(m_caster, searcher, max_range);
                }
                if (tempTargetUnitMap.empty())
                    break;
...
void Spell::FillAreaTargets(UnitList &targetUnitMap, float x, float y, float radius, SpellNotifyPushType pushType, SpellTargets spellTargets)
{
    ...
    cell.SetNoCreate();
    Diamond::SpellNotifierCreatureAndPlayer notifier(*this, targetUnitMap, radius, pushType, spellTargets);
