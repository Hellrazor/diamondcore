    ...
    if(m_caster_guid == m_target->GetGUID())
    {
        ...
        if( !caster->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            switch(m_areaAuraType)
            {
                ...
                {
                    Diamond::AnyFriendlyUnitInObjectRangeCheck u_check(caster, m_radius);
                    Diamond::UnitListSearcher<Diamond::AnyFriendlyUnitInObjectRangeCheck> searcher(caster,targets, u_check);
                    Cell::VisitAllObjects(caster, searcher, m_radius);
                    break;
                }
                case AREA_AURA_ENEMY:
                {
                    Diamond::AnyAoETargetUnitInObjectRangeCheck u_check(caster, m_radius); // No GetCharmer in searcher
                    Diamond::UnitListSearcher<Diamond::AnyAoETargetUnitInObjectRangeCheck> searcher(caster, targets, u_check);
                    Cell::VisitAllObjects(caster, searcher, m_radius);
                    break;
                }
...
void Aura::HandleChannelDeathItem(bool apply, bool Real)
{
    if(Real && !apply)
    {
        ...
        if( spellInfo->EffectItemType[m_effIndex] == 6265 &&
            (victim->getLevel() <= Diamond::XP::GetGrayLevel(caster->getLevel()) ||
            ...
            switch (spell->Id)
            {
                ...
                case 51690:
                {
                    ...
                    std::list<Unit*> targets;
                    {
                        ...
                        Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(m_target, m_target, radius);
                        Diamond::UnitListSearcher<Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(m_target, targets, u_check);
