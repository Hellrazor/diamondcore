...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket & recv_data)
{
    ...
    if(!GetPlayer()->isGameMaster())
    {
        ...
        if(missingLevel || missingItem || missingKey || missingQuest)
        {
            ...
            if(missingItem)
                SendAreaTriggerMessage(GetString(LANG_LEVEL_MINREQUIRED_AND_ITEM), at->requiredLevel, ObjectMgr::GetItemPrototype(missingItem)->Name1);
            else if(missingKey)
                ...
                SendAreaTriggerMessage(GetString(LANG_LEVEL_MINREQUIRED), missingLevel);
            return;
        }
    }
...
void WorldSession::HandleMoveTimeSkippedOpcode( WorldPacket & recv_data )
{
    ...
    /*
        ...
        /// TODO
        must be need use in DiamondCore
