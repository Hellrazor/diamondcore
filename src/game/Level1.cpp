...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetString(LANG_VISIBLE) : GetString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleNamegoCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(),GetString(LANG_OFFLINE));
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if(!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetString(LANG_ON) : GetString(LANG_OFF));
        return true;
    }
...
bool ChatHandler::HandleTeleNameCommand(const char * args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetString(LANG_OFFLINE), tele->name.c_str());
