...
// 2^n values, Player::m_isunderwater is a bitmask. These are DiamondCore internal values, they are never send to any client
...
class DIAMOND_DLL_SPEC PlayerTaxi
{
    public:
        PlayerTaxi();
        ~PlayerTaxi() {}
...
class DIAMOND_DLL_SPEC Player : public Unit, public GridPlayer
{
    ...
    public:
        ...
        static bool GetDeletedCharacterGUIDs(std::list<uint32>& foundGUIDs, std::string searchString = "");

        static uint32 GetAccountIdByGUID(uint64 guid);
