...
bool ChatHandler::HandleDebugMoveMapCommand(const char* args)
{
    ...
    CellPair pair(Diamond::ComputeCellPair( m_session->GetPlayer()->GetPositionX(), m_session->GetPlayer()->GetPositionY()) );
    Cell cell(pair);
...
bool ChatHandler::HandleDebugSetValueCommand(const char* args)
{
    ...
    if(isint32)
    {
        ...
        sLog.outDebug(GetString(LANG_SET_UINT), GUID_LOPART(guid), Opcode, iValue);
    ...
    else
    {
        ...
        sLog.outDebug(GetString(LANG_SET_FLOAT), GUID_LOPART(guid), Opcode, fValue);
...
bool ChatHandler::HandleDebugGetValueCommand(const char* args)
{
    ...
    if(isint32)
    {
        ...
        sLog.outDebug(GetString(LANG_GET_UINT), GUID_LOPART(guid), Opcode, iValue);
        PSendSysMessage(LANG_GET_UINT_FIELD, GUID_LOPART(guid), Opcode,    iValue);
    }
    else
    {
        fValue = target->GetFloatValue( Opcode );
        sLog.outDebug(GetString(LANG_GET_FLOAT), GUID_LOPART(guid), Opcode, fValue);
...
bool ChatHandler::HandleDebugMod32ValueCommand(const char* args)
{
    ...
    sLog.outDebug(GetString(LANG_CHANGE_32BIT), Opcode, Value);
