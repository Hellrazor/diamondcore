...
bool ChatHandler::HandleGoCreatureCommand(const char* args)
{
    ...
    else
    {
        ...
        if(!guid)
        {
            ...
            whereClause << ", creature_template WHERE creature_spawns.id = creature_template.entry AND creature_template.name "_LIKE_" '" << name << "'";
        }
        else
        {
    ...
    QueryResult *result = WorldDatabase.PQuery("SELECT position_x,position_y,position_z,orientation,map FROM creature_spawns %s", whereClause.str().c_str() );
    if (!result)
    {
...
bool ChatHandler::HandleGameObjectTargetCommand(const char* args)
{
    ...
    if(*args)
    {
        ...
        if(id)
            result = WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, orientation, map, (POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + POW(position_z - '%f', 2)) AS order_ FROM gameobject_spawns WHERE map = '%i' AND id = '%u' ORDER BY order_ ASC LIMIT 1",
        ...
        else
        {
            ...
            result = WorldDatabase.PQuery(
                ...
                "FROM gameobject_spawns,gameobject_template WHERE gameobject_template.entry = gameobject_spawns.id AND map = %i AND name "_LIKE_" "_CONCAT3_("'%%'","'%s'","'%%'")" ORDER BY order_ ASC LIMIT 1",
    ...
    else
    {
        ...
        result = WorldDatabase.PQuery("SELECT gameobject_spawns.guid, id, position_x, position_y, position_z, orientation, map, "
            "(POW(position_x - %f, 2) + POW(position_y - %f, 2) + POW(position_z - %f, 2)) AS order_ FROM gameobject_spawns "
            "LEFT OUTER JOIN game_event_gameobject on gameobject_spawns.guid=game_event_gameobject.guid WHERE map = '%i' %s ORDER BY order_ ASC LIMIT 10",
...
bool ChatHandler::HandleGameObjectAddCommand(const char* args)
{
    ...
    sLog.outDebug(GetString(LANG_GAMEOBJECT_CURRENT), gInfo->name, db_lowGUID, x, y, z, o);
...
bool ChatHandler::HandleGameObjectNearCommand(const char* args)
{
    ...
    QueryResult *result = WorldDatabase.PQuery("SELECT guid, id, position_x, position_y, position_z, map, "
        ...
        "FROM gameobject_spawns WHERE map='%u' AND (POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + POW(position_z - '%f', 2)) <= '%f' ORDER BY order_",
...
bool ChatHandler::HandleModifyRepCommand(const char * args)
{
    ...
    if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
    {
        ...
        {
            std::string rank = GetString(ReputationRankStrIndex[r]);
            if(rank.empty())
                continue;
...
bool ChatHandler::HandleNpcAddMoveCommand(const char* args)
{
    ...
    */
    ...
    WorldDatabase.PExecuteLog("UPDATE creature_spawns SET MovementType = '%u' WHERE guid = '%u'", WAYPOINT_MOTION_TYPE,lowguid);
    if(pCreature)
    {
...
bool ChatHandler::HandleNpcMoveCommand(const char* args)
{
    ...
    WorldDatabase.PExecuteLog("UPDATE creature_spawns SET position_x = '%f', position_y = '%f', position_z = '%f', orientation = '%f' WHERE guid = '%u'", x, y, z, o, lowguid);
...
bool ChatHandler::HandleNpcSpawnDistCommand(const char* args)
{
    ...
    WorldDatabase.PExecuteLog("UPDATE creature_spawns SET spawndist=%f, MovementType=%i WHERE guid=%u",option,mtype,u_guidlow);
...
bool ChatHandler::HandleNpcSpawnTimeCommand(const char* args)
{
    ...
    WorldDatabase.PExecuteLog("UPDATE creature_spawns SET spawntimesecs=%i WHERE guid=%u",i_stime,u_guidlow);
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, security, last_ip.c_str(), last_login.c_str(), latency);
...
bool ChatHandler::HandleWpShowCommand(const char* args)
{
        ...
        if(result2)
        {
            ...
            do
            {
                ...
                if(!pCreature)
                {
                    ...
                    hasError = true;
                    WorldDatabase.PExecuteLog("DELETE FROM creature_spawns WHERE guid = '%u'", wpguid);
                }
                else
                {
    ...
    if(show == "off")
    {
        QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM creature_spawns WHERE id = '%d'", VISUAL_WAYPOINT);
        if(!result)
        {
        ...
        do
        {
            ...
            if(!pCreature)
            {
                ...
                hasError = true;
                WorldDatabase.PExecuteLog("DELETE FROM creature_spawns WHERE guid = '%u'", guid);
            }
            else
            {
                pCreature->DeleteFromDB();
...
bool ChatHandler::HandleEventListCommand(const char* args)
{
    ...
    char const* active = GetString(LANG_ACTIVE);
    char const* inactive = GetString(LANG_FACTION_INACTIVE);
    char const* state = "";
...
bool ChatHandler::HandleEventInfoCommand(const char* args)
{
    ...
    char const* activeStr = active ? GetString(LANG_ACTIVE) : "";
