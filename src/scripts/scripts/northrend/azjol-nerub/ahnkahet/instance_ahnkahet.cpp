...
struct DIAMOND_DLL_DECL instance_ahnkahet : public ScriptedInstance
{
    ...
    {
        debug_log("DS: Instance Ahn'Kahet: SetData received for type %u with data %u",uiType,uiData);
        ...
        switch(uiType)
        {
            ...
            case TYPE_AMANITAR:
                ...
                break;
            default:
                error_log("DS: Instance Ahn'Kahet: ERROR SetData = %u for type %u does not exist/not implemented.",uiType,uiData);
                break;
        }
