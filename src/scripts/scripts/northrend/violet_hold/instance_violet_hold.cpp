...
{
    debug_log("DS: instance_violet_hold: SetData got type % u, data %u.", uiType, uiData);
...
void instance_violet_hold::SetPortalId()
{
    if (IsCurrentPortalForTrash())
    {
        ...
        debug_log("DS: instance_violet_hold: SetPortalId %i, old was id %u.", iTemp, m_uiPortalId);

        m_uiPortalId = iTemp;
    }
    else
    {
        debug_log("DS: instance_violet_hold: SetPortalId %u (is boss), old was id %u.", m_uiMaxCountPortalLoc, m_uiPortalId);
...
void instance_violet_hold::SetRandomBosses()
{
    ...
    for(std::list<uint32>::iterator itr = m_lRandomBossList.begin(); itr != m_lRandomBossList.end(); ++itr)
        debug_log("DS: instance_violet_hold random boss is entry %u", *itr);
}
