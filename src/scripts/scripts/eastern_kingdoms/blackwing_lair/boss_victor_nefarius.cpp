...
struct DIAMOND_DLL_DECL boss_victor_nefariusAI : public ScriptedAI
{
    ...
    void BeginEvent(Player* target)
    {
        ...
        //Diamond::Singleton<MapManager>::Instance().GetMap(m_creature->GetMapId(), m_creature)->GetPlayers().begin();
        /*
        ...
        if (SpawnedAdds < 42)
        {
            ...
            if (AddSpawnTimer < diff)
            {
                ...
                if (SpawnedAdds >= 42)
                {
                    ...
                    else error_log("DS: Blackwing Lair: Unable to spawn nefarian properly.");
                }
