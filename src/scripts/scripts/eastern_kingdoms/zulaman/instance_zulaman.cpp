...
struct DIAMOND_DLL_DECL instance_zulaman : public ScriptedInstance
{
    ...
    {
        debug_log("DS: Instance Zulaman: SetData received for type %u with data %u",uiType,uiData);
        ...
        switch(uiType)
        {
            ...
            case TYPE_RAND_VENDOR_2:
                ...
                break;
            default:
                error_log("DS: Instance Zulaman: ERROR SetData = %u for type %u does not exist/not implemented.",uiType,uiData);
                break;
        }
    ...
    void Update(uint32 uiDiff)
    {
        if (GetData(TYPE_EVENT_RUN) == IN_PROGRESS)
        {
            if (m_uiEventTimer <= uiDiff)
            {
                ...
                {
                    debug_log("DS: Instance Zulaman: event time reach end, event failed.");
                ...
                --m_uiEventMinuteStep;
                DoUpdateWorldState(WORLD_STATE_COUNTER, m_uiEventMinuteStep);
                debug_log("DS: Instance Zulaman: minute decrease to %u.",m_uiEventMinuteStep);
