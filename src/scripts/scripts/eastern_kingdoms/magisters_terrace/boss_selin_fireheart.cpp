...
struct DIAMOND_DLL_DECL boss_selin_fireheartAI : public ScriptedAI
{
    boss_selin_fireheartAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        if (m_pInstance)
        {
            ...
            for(uint8 i = 0; i < size; ++i)
            {
                ...
                debug_log("DS: Selin: Adding Fel Crystal " UI64FMTD " to list", guid);
    ...
    void Reset()
    {
        if (m_pInstance)
        {
            ...
            for(std::list<uint64>::iterator itr = Crystals.begin(); itr != Crystals.end(); ++itr)
            {
                ...
                {
                    if (!pUnit->isAlive())
                        ((Creature*)pUnit)->Respawn();      // Let DiamondCore handle setting death state, etc.
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == POINT_MOTION_TYPE && id == 1)
        {
            ...
            else
            {
                ...
                error_log("DS: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                DrainingCrystal = false;
            }
        }
    }
...
struct DIAMOND_DLL_DECL mob_fel_crystalAI : public ScriptedAI
{
