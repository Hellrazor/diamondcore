            ...
            if (m_uiPhaseTimer < uiDiff)
            {
                if (m_uiPhase == PHASE_DRESSUP)
                {
                    if (m_pToTransform)
                    {
                        ...
                        error_log("DS: npc_unworthy_initiate does not have any spell associated with model");
...
//  because DiamondCore          DoCast(m_creature, SPELL_MOGRAINE_CHARGE); // jumping charge
