...
struct DIAMOND_DLL_DECL instance_shadow_labyrinth : public ScriptedInstance
{
        ...
        switch(pCreature->GetEntry())
        {
            ...
            case 18796:
                if (pCreature->isAlive())
                {
                    ...
                    debug_log("DS: Shadow Labyrinth: counting %u Fel Overseers.", m_uiFelOverseerCount);
                }
                break;
        }
    }
    ...
    void SetData(uint32 uiType, uint32 uiData)
    {
        switch(uiType)
        {
            ...
            case TYPE_OVERSEER:
                if (uiData != DONE)
                {
                    error_log("DS: Shadow Labyrinth: TYPE_OVERSEER did not expect other data than DONE");
                    return;
                }
                if (m_uiFelOverseerCount)
                {
                    ...
                    if (m_uiFelOverseerCount)
                        debug_log("DS: Shadow Labyrinth: %u Fel Overseers left to kill.", m_uiFelOverseerCount);
                    else
                    {
                        m_auiEncounter[1] = DONE;
                        debug_log("DS: Shadow Labyrinth: TYPE_OVERSEER == DONE");
                    }
                }
                break;
