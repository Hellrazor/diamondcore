...
struct DIAMOND_DLL_DECL boss_kaelthasAI : public ScriptedAI
{
    ...
    void StartEvent()
    {
        ...
        {
            error_log("DS: Kael'Thas One or more advisors missing, Skipping Phases 1-3");
            ...
            case PHASE_3_ADVISOR_ALL:
            {
                if (m_uiPhaseSubphase == 0)
                {
                    ...
                    for (uint32 i = 0; i < MAX_ADVISORS; ++i)
                    {
                        ...
                        if (!pAdvisor)
                            error_log("DS: Kael'Thas Advisor %u does not exist. Possibly despawned? Incorrectly Killed?", i);
                        else
            ...
            case 6:
            {
                ...
                if (!m_bInGravityLapse && !m_bChainPyros && m_uiPhase != 5)
                {
                    ...
                    if (m_uiMindControl_Timer < uiDiff)
                    {
                        ...
                        {
                            debug_log("DS: Kael'Thas mind control not supported.");
