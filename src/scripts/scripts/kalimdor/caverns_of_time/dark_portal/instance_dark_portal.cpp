...
struct DIAMOND_DLL_DECL instance_dark_portal : public ScriptedInstance
{
        ...
        switch(type)
        {
            case TYPE_MEDIVH:
                ...
                else
                {
                    if (data == IN_PROGRESS)
                    {
                        debug_log("DS: Instance Dark Portal: Starting event.");
                        InitWorldState();
                    ...
                    if (data == DONE)
                    {
                        ...
                        debug_log("DS: Instance Dark Portal: Event completed.");
    ...
    Creature* SummonedPortalBoss(Creature* pSource)
    {
        ...
        debug_log("DS: Instance Dark Portal: Summoning rift boss entry %u.",entry);
        ...
        debug_log("DS: Instance Dark Portal: what just happened there? No boss, no loot, no fun...");
        return NULL;
    }
