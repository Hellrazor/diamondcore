...
struct DIAMOND_DLL_DECL npc_twiggy_flatheadAI : public ScriptedAI
{
    ...
    bool CanStartEvent(Player* pPlayer)
    {
        ...
        debug_log("DS: npc_twiggy_flathead event already in progress, need to wait.");
        return false;
    }
    ...
    void SetChallengers()
    {
        for(uint8 i = 0; i < 6; ++i)
        {
            ...
            if (!pCreature)
            {
                debug_log("DS: npc_twiggy_flathead event cannot summon challenger as expected.");
                continue;
            }
...
bool AreaTrigger_at_twiggy_flathead(Player* pPlayer, AreaTriggerEntry* pAt)
{
    if (!pPlayer->isDead() && pPlayer->GetQuestStatus(QUEST_AFFRAY) == QUEST_STATUS_INCOMPLETE)
    {
        ...
        if (((npc_twiggy_flatheadAI*)pCreature->AI())->CanStartEvent(pPlayer))
            return false;                                   //ok to let DiamondCore process further
        else
            return true;
    }
    return true;
