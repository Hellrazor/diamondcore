...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        debug_log("DS: FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(const uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !m_creature->getVictim())
    {
        if (m_uiUpdateFollowTimer < uiDiff)
        {
            ...
            {
                debug_log("DS: FollowerAI is set completed, despawns.");
            ...
            if (Player* pPlayer = GetLeaderForFollower())
            {
                ...
                {
                    debug_log("DS: FollowerAI is returning to leader.");
            ...
            {
                debug_log("DS: FollowerAI failed because player/group was to far away or not found");
                m_creature->ForcedDespawn();
                return;
            }
...
void FollowerAI::StartFollow(Player* pLeader, uint32 uiFactionForFollower, const Quest* pQuest)
{
    if (m_creature->getVictim())
    {
        debug_log("DS: FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        error_log("DS: FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        debug_log("DS: FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    debug_log("DS: FollowerAI start follow %s (GUID " UI64FMTD ")", pLeader->GetName(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* pGroup = pLeader->GetGroup())
            {
                for(GroupReference* pRef = pGroup->GetFirstMember(); pRef != NULL; pRef = pRef->next())
                {
                    ...
                    {
                        debug_log("DS: FollowerAI GetLeader changed and returned new leader.");
    ...
    debug_log("DS: FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
