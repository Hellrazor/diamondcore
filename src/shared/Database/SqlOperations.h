...
class SqlResultQueue : public ACE_Based::LockedQueue<Diamond::IQueryCallback* , ACE_Thread_Mutex>
{
    public:
        SqlResultQueue() {}
        void Update();
...
class SqlQuery : public SqlOperation
{
    private:
        const char *m_sql;
        Diamond::IQueryCallback * m_callback;
...
class SqlQueryHolder
{
    ...
    public:
        ...
        bool Execute(Diamond::IQueryCallback * callback, SqlDelayThread *thread, SqlResultQueue *queue);
};
...
class SqlQueryHolderEx : public SqlOperation
{
    private:
        ...
        SqlResultQueue * m_queue;
    public:
        SqlQueryHolderEx(SqlQueryHolder *holder, Diamond::IQueryCallback * callback, SqlResultQueue * queue)
