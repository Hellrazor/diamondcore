...
void SqlResultQueue::Update()
{
    ...
    Diamond::IQueryCallback* callback;
    while (next(callback))
    {
        callback->Execute();
        delete callback;
...
bool SqlQueryHolder::Execute(Diamond::IQueryCallback * callback, SqlDelayThread *thread, SqlResultQueue *queue)
{
...
bool SqlQueryHolder::SetQuery(size_t index, const char *sql)
{
    ...
    m_queries[index] = SqlResultPair(_strdup(sql), (QueryResult*)NULL);
    return true;
}
