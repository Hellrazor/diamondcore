...
bool ChatHandler::HandleCharacterDeletedListCommand(const char* args)
{
    ...
    if(resultChar)
    {
        ...
        do
        {
            ...
            std::string deleteDate = fields[3].GetCppString();

            std::string accountName;
            sAccountMgr.GetName (accountId, accountName);

            PSendSysMessage(LANG_CHARACTER_DELETED_LIST_LINE, guid, name.c_str(), accountName.c_str(), accountId, deleteDate.c_str());
...
bool ChatHandler::HandleCharacterDeletedRestoreCommand(const char* args)
{
    ...
    char* searchString = strtok((char*)args, " ");
    ...
    else if (foundGUIDs.size() == 1)
        CharacterDatabase.PQuery("UPDATE characters SET name='%s', account=deleteInfos_Account, deleteDate=NULL, deleteInfos_Name=NULL, deleteInfos_Account=NULL WHERE deleteDate IS NOT NULL AND guid IN (%s)", newName, ss.str().c_str());
    else
        SendSysMessage("ERROR: You can only assign a new name if you have only selected one character!");
